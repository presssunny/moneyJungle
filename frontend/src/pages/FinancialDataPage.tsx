...
export default function FinancialDataPage(){
 ...
 <AsyncSection resource={state} errorTitle="לא ניתן לטעון את המקורות" skeleton={<Loading/>}>{data=><>{data.picture&&<FinancialPicturePanel picture={data.picture} onSaved={state.reload}/>}<CoveragePanel data={data} onSaved={state.reload} refreshing={state.loading}/></>}</AsyncSection>
 </div>;
}
