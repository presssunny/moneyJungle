...
export default function CheckInPage(){
 ...
 {step===2&&<Card title="הבנה — מה השתנה?">{data.comparison.baseline?<p>אין עדיין בדיקה קודמת להשוואה. הנתונים הנוכחיים יהיו נקודת ההתחלה.</p>:<><p>{data.comparison.added??"לא ידוע"} רשומות חדשות במערכת, מתוכן {data.comparison.late??"לא ידוע"} מתאריכים שקדמו לבדיקה הקודמת.</p><p>{data.comparison.changed??"לא ידוע"} רשומות תוקנו · {data.comparison.removed??"לא ידוע"} הוסרו.</p><p className="text-muted">מידע היסטורי שנקלט כעת אינו הוצאה חדשה השבוע.</p>{data.comparison.cashChange!==null&&<p>שינוי ביתרות הבנק: {formatCurrency(data.comparison.cashChange)}</p>}</>}<p className="text-muted">ההשוואה המפורטת מכסה עד 1,000 רשומות בשלושת החודשים האחרונים. מידע ישן נשמר כסיכום שינוי.</p>{data.comparison.limited&&<p>כמות הרשומות חורגת מהחלון המפורט; ספירת שינויים מלאה אינה זמינה.</p>}{data.comparison.historyChanged&&<p>גם מידע מחוץ לחלון המפורט השתנה. אין לפרש זאת כהוצאה חדשה השבוע.</p>}<h3>חיובים קרובים וחובות פתוחים</h3><ul>{data.status.upcoming.slice(0,5).map(e=><li key={e.key}>{e.name} · {formatDate(e.date)} · {e.amount===null?'סכום לא ידוע':formatCurrency(e.amount)}</li>)}</ul>{data.status.blockers.length>0&&<p>התמונה חלקית — אין להסיק מהשינוי ברשומות שינוי מלא במצב הכספי.</p>}</Card>}
