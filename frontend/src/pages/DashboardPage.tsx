...
export default function DashboardPage(){
 ...
 return <div className="journey-page home-workspace">
  ...
  <Card className="home-monthly" title={`מה נרשם ב${formatMonthKey(monthKey)}?`}>
  ...
  <AsyncSection resource={state} errorTitle="לא ניתן לבדוק את תמונת הכסף" skeleton={<Loading/>}>{data=><>
   <Card className="home-status" title="איפה אנחנו עומדים היום?">
    ...
    <MetricExplanation metric="allowance" version={data.dataVersion}><p>{data.allowance.formula}</p><p>שמור בצד — כרית ביטחון, חיסכון וחיובי כרטיס של החודש הבא: {formatCurrency(data.allowance.reserves)} · הוצאות חיוניות שטרם נרשמו: {formatCurrency(data.allowance.essentialReserve)}</p><ul>{data.balances.map(b=><li key={b.id}>{b.name}: {b.explanation}</li>)}{data.allowance.assumptions.map(a=><li key={a}>{a}</li>)}{data.blockers.map(b=><li key={b}>{b}</li>)}</ul><Link to="/commitments">פירוט ההתחייבויות</Link></MetricExplanation>
    <MetricExplanation title="מאיפה מגיעות יתרות הבנק" metric="cash" version={data.dataVersion}/>
   </Card>
   {data.picture&&<FinancialPicturePanel picture={data.picture} compact onSaved={state.reload}/>}
   <Card className="home-upcoming" title="מה צפוי בקרוב?" action={<Link to="/commitments">לכל ההתחייבויות</Link>}>
    {(()=>{const upcoming=data.upcoming;return upcoming.length?<ul className="journey-list">{upcoming.slice(0,5).map(e=><li key={e.key}><Link to={e.to}>{e.name} · {formatDate(e.date)}{e.date<data.today&&" · בפיגור"}</Link><span>{e.amount===null?'סכום לא ידוע':formatCurrency(e.amount)}{!e.decision&&' · לבדיקה'}</span></li>)}</ul>:<p>לא רשומים חיובים לשבעת הימים הקרובים. ייתכנו חיובים שטרם נוספו.</p>;})()}
   </Card>
  </>}</AsyncSection>
  <Card className="home-actions" title="מה כדאי לעשות עכשיו?"><AsyncSection resource={state} errorTitle="לא ניתן לטעון פעולות חשובות" skeleton={<Loading/>}>{data=><>
