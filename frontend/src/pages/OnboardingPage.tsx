...
export default function OnboardingPage(){
 ...
 // Completion never confirms coverage on the user's behalf — that has to
 // happen where the user actually sees each source, in step 3 (/data). This
 // button only fires once the server already has that real acknowledgement.
 ...
 <AsyncSection resource={state} errorTitle="לא ניתן לטעון את ההתקדמות" skeleton={<Loading/>}>{data=><Card title="בדיקת התמונה הראשונה"><p>{data.issues.filter(i=>i.blocking).length} פריטים מחייבים השלמה לפני סיום.</p>{data.blockers.length>0&&<details open><summary>מגבלות התמונה הנוכחית</summary><ul>{data.blockers.map(b=><li key={b}>{b}</li>)}</ul></details>}{!data.coverageAcknowledged&&<p>יש לעבור על <Link to="/data">בדיקת המקורות וההתחייבויות</Link> (שלב 3) ולאשר שם שהמידע עדכני, לפני סיום ההיכרות.</p>}<p><label><input type="checkbox" checked={reviewed} onChange={e=>setReviewed(e.target.checked)}/> בדקתי את הנתונים והבנתי איזה מידע עדיין חסר</label></p>{data.profile.scope?.manualOnly&&<p><label><input type="checkbox" checked={empty} onChange={e=>setEmpty(e.target.checked)}/> אין כרגע פעילות לרישום — אתחיל להזין בהמשך</label></p>}<Button disabled={busy||!reviewed||!data.profile.scope||!data.coverageAcknowledged||data.issues.some(i=>i.blocking)} onClick={finish}>סיום ההיכרות</Button></Card>}</AsyncSection>
