...
import { getFinancialStatus, finishOnboarding, confirmCoverage } from "../services/journey.service";
export default function OnboardingPage(){
 ...
 // The checkbox alone is a local UI hint; the server only accepts completion
 // once a real coverage acknowledgement was recorded against current data.
 async function finish(){setBusy(true);setError('');try{if(!state.data)throw new Error('missing status');await confirmCoverage(state.data.dataVersion,state.data.sources.map(s=>s.key));await finishOnboarding(empty);navigate('/',{replace:true});}catch(e){setError(apiErrorMessage(e));}finally{setBusy(false);}}
