...
An alert is raised only for a group whose records all fall in the current month, but withdrawal spans the whole 90-day scan window and is not bounded by the month the alert began in — a finding resolved on the 1st is no longer live whatever day it was raised. The open-group list used for withdrawal is the uncapped one: the 50-group response cap is a display limit, and reading it as "everything still open" would withdraw live findings and re-raise them later with their read state lost. A scan that hit its row limit withdraws nothing, since a truncated scan cannot prove a group is gone.
...
`Income` now carries `source` (`manual` | `bank_import`, backfilled from bank links). The duplicate scan reads manual incomes only and `remove_manual` refuses a statement income outright. The old amount/date search for an unlinked bank row remains only for incomes created before the column existed. Issuer follow-up is a note to the household, not an action: nothing is sent to the card issuer, and no one is reminded after the first time the entry appears in the tracking list. Decision history is kept indefinitely and has no export. Restoring a removed row depends on the archived snapshot, so a restore attempted long after the surrounding data moved on will correctly refuse more often than it succeeds.

## Questions (added 2026-09-26)

`POST /api/household-assistant/ask` `{question, consent, documentId?}` answers from recorded money. A local Hebrew router covers month totals, category spend, today's allowance, upcoming charges, loans, goals and net worth, and — for a document — its summary and interest. Only when it cannot route, and only with `consent: true` for that question, the question text alone is sent to the configured model, which may return nothing but an intent from the server's catalogue; anything else is "unanswered" with examples. The model never receives a figure, record, category list or file. Every figure comes from the owning service (`monthTotals`, `spentByCategory`, `financialStatus`, `loansService`, `savingsService`, the netWorth metric, `documentBreakdown`), and each answer lists its limitations. Statement interest is shown gross (charged, credited) and then net; a loan schedule's interest is worded as planned, never charged. Read-only: not in the activity log, not broadcast as a mutation. Without a provider key, model routing is the only part that does not run.
